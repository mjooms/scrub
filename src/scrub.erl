%%%-------------------------------------------------------------------
%%% Updated: 20  Sep 2013 by Matthijs Ooms <m.j.ooms@gmail.com>
...
%%% modified (Mo, Sep 2013): Update to scrub.
%%% modified (WdJ, May 2007): deal with imports in the WSDL.
%%% modified (WdJ, August 2007): the WSDL can contain more than 1 schema
%%% copied from scrub_soap_lib (Kaloyan Dimitrov, February 2012): to be used for soap12 calls
...
-include("../include/scrub_soap.hrl").
...
-record(scrub_soap_config, {atts, xsd_path,  user_module, wsdl_file, add_files}).
...
initModelFile(ConfigFile) ->
    ...
    #scrub_soap_config{xsd_path = XsdPath,
		      wsdl_file = Wsdl,
...
make_request_body(Content, AttachedFiles, _Operation) ->
    {"application/dime",
     scrub_dime:encode("<?xml version=\"1.0\" encoding=\"utf-8\"?>" ++ Content,
                      AttachedFiles)}.
...
findHeader(Label, Headers) ->
    findHeader0(scrub:to_lower(Label), Headers).

findHeader0(_Label, []) ->
    undefined;
findHeader0(Label, [{_,_,Hdr,_,Val}|T]) ->
    case {Label, scrub:to_lower(Hdr)} of
	{X,X} -> Val;
...
config_file_xsd() ->
    "<xs:schema xmlns:xs=\"http://www.w3.org/2001/XMLSchema\">"
        "  <xs:element name=\"scrub_soap_config\">"
        "     <xs:complexType>"
        "       <xs:sequence>"
